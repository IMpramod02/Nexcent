...
async function createUser(user) {
  ...
  setTimeout(async () => {
    await writeJsonFile(dbPath, db_new);
  }, 100);
  ...
  switch (req.path) {
      ...
      try {
        ...
        if (username === undefined || password === undefined) {
          res
            .status(400)
            .json({
              message: `${req.method} Request body does not contain username or password`,
            });
          break;
        }
    ...
    case '/register': {
      ...
      try {
        ...
        const dbUser = (await getUsers()).find(
      ...
      if (userIdRegex.test(req.path)) {
        if (
          ...
          try {
            ...
            if (
              (req.path === `/users/${verifyTokenResult['userId']}` ||
                req.path === `/users/${verifyTokenResult['userId']}/`) &&
              !(['GET', 'PATCH'].indexOf(req.method) >= 0)
            ) {
              const status = 403;
              const message = 'UnAuthorized to access data';
              res.status(status).json({ status, message });
